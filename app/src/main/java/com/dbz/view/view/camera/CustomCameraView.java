...
import com.ym.chat.widget.camera.listener.OnVideoPlayPrepareListener;
...
public class CustomCameraView extends FrameLayout {
    ...
    public void initView() {
        ...
        mPhoto = view.findViewById(R.id.photoView);
...
//        mCameraView.setSnapshotMaxWidth(1080);
        mCameraView.setSnapshotMaxHeight(ScreenUtils.getScreenHeight());
        mCameraView.setSnapshotMaxWidth(ScreenUtils.getScreenWidth());
...
//        SizeSelector height = SizeSelectors.maxHeight(2160);
        ...
        SizeSelector height = SizeSelectors.maxHeight(ScreenUtils.getScreenHeight());
        ...
        mCameraView.addCameraListener(new CameraListener() {
            ...
            public void onCameraError(@NonNull CameraException exception) {
                ...
                ActivityUtils.finishActivity(CameraViewActivity.class);
...
//                }
            ...
            public void onPictureTaken(@NonNull PictureResult result) {
                ...
                LogUtils.e("------onPictureTaken result = " + result);
                result.toFile(initTakePicPath(mContext), file -> {
                    ...
                    photoFile = file;
                    // 获取图片宽高
                    MediaExtraInfo extraInfo = MediaUtils.getImageSize(getContext(), photoFile.getAbsolutePath());
                    // 如果是横图 那么纵横比显示
                    if (extraInfo.getWidth() > extraInfo.getHeight()) {
...
//                mCameraView.takeVideoSnapshot(initStartRecordingPath(mContext));
                mCameraView.postDelayed(() -> mCameraView.takeVideoSnapshot(initStartRecordingPath(mContext)), 20);
            }
            ...
            public void recordShort(final long time) {
                ...
                mCaptureLayout.setTextWithAnimation(StringUtils.getString(R.string.recording_too_short));
                mCameraView.stopVideo();
            }
            ...
            public void recordZoom(float zoom) {
                // 获取当前的摄像头的缩放比例
                float ratio = zoom / (ScreenUtils.getScreenHeight() - mCaptureLayout.getButtonSize() * 2);
                mCameraView.setZoom(ratio);
            }

            @Override
            public void recordError() {
                if (flowCameraListener != null) {
                    flowCameraListener.onError(0, "错误", null);
                }
            }
        });
        //确认 取消
     ...
     * @param absolutePath
     */
    private LocalMedia buildLocalMedia(String absolutePath) {
        LocalMedia media = LocalMedia.generateLocalMedia(mContext, absolutePath);
        ...
        if (PictureMimeType.isHasImage(media.getMimeType())) {
            BitmapUtils.rotateImage(mContext, absolutePath);
        }
        return media;
    }
